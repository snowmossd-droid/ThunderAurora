...
import thunder.aurora.core.Core;
...
import thunder.aurora.injection.accesors.ILivingEntity;
import thunder.aurora.features.modules.Module;
import thunder.aurora.features.modules.client.HudEditor;
...
import thunder.aurora.utility.interfaces.IOtherClientPlayerEntity;
import thunder.aurora.utility.math.MathUtility;
import thunder.aurora.utility.player.InteractionUtility;
import thunder.aurora.utility.player.InventoryUtility;
import thunder.aurora.utility.player.PlayerUtility;
...
import thunder.aurora.utility.render.animation.CaptureMark;
...
import static thunder.aurora.utility.math.MathUtility.random;
...
public class Aura extends Module {
    ...
    public final Setting<ESP> esp = new Setting<>("ESP", ESP.ThunderAurora);
    ...
    public final Setting<SettingGroup> espGroup = new Setting<>("ESPSettings", new SettingGroup(false, 0), v -> esp.is(ESP.ThunderAuroraV2));
    public final Setting<Integer> espLength = new Setting<>("ESPLength", 14, 1, 40, v -> esp.is(ESP.ThunderAuroraV2)).addToGroup(espGroup);
    public final Setting<Integer> espFactor = new Setting<>("ESPFactor", 8, 1, 20, v -> esp.is(ESP.ThunderAuroraV2)).addToGroup(espGroup);
    public final Setting<Float> espShaking = new Setting<>("ESPShaking", 1.8f, 1.5f, 10f, v -> esp.is(ESP.ThunderAuroraV2)).addToGroup(espGroup);
    public final Setting<Float> espAmplitude = new Setting<>("ESPAmplitude", 3f, 0.1f, 8f, v -> esp.is(ESP.ThunderAuroraV2)).addToGroup(espGroup);
    ...
    public void onUpdate(PlayerUpdateEvent e) {
        ...
        if (mc.player.isUsingItem() && pauseWhileEating.getValue())
            return;
        if(pauseBaritone.getValue() && ThunderAurora.baritone){
    ...
    public float getAttackCooldownProgressPerTick() {
        return (float) (1.0 / mc.player.getAttributeValue(EntityAttributes.GENERIC_ATTACK_SPEED) * (20.0 * ThunderAurora.TICK_TIMER * (tpsSync.getValue() ? Managers.SERVER.getTPSFactor() : 1f)));
    }
    ...
    public void onRender3D(MatrixStack stack) {
        ...
        switch (esp.getValue()) {
            ...
            case ThunderAuroraV2 -> Render3DEngine.renderGhosts(espLength.getValue(), espFactor.getValue(), espShaking.getValue(), espAmplitude.getValue(), target);
            case ThunderAurora -> Render3DEngine.drawTargetEsp(stack, target);
    ...
    public enum ESP {
        Off, ThunderAurora, NurikZapen, CelkaPasta, ThunderAuroraV2, Orbit, Spectre, Wraith
    }
