...
import thunder.aurora.core.InputBlocker;
import thunder.aurora.core.Managers;
import thunder.aurora.core.manager.client.AsyncManager;
import thunder.aurora.core.manager.client.ModuleManager;
import thunder.aurora.events.impl.EventFixVelocity;
import thunder.aurora.events.impl.EventKeyboardInput;
...
public class AutoCart extends Module {
   ...
   private final Setting<Integer> startDelay = new Setting<>("BowDelay", 25, 0, 60);
   private final Setting<Integer> drawDelay = new Setting<>("DrawDelay", 600, 100, 2000);
   private final Setting<Integer> releaseDelay = new Setting<>("ReleaseDelay", 100, 0, 500);
   private final Setting<Bind> activeBind = new Setting<>("Keybind", new Bind(-1, false, false));
   private final Setting<Integer> delay = new Setting<>("FireDelay", 25, 0, 100);
      ...
      if (!fullNullCheck() && !this.cartAuraExecuting) {
         ...
         if (popTarget != mc.player) {
            ...
            boolean isNearbyPlayer = this.nearbyPlayers.getValue()
               && !thunder.aurora.core.Managers.FRIEND.isFriend(popTarget)
