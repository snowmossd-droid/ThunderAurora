...
import thunder.aurora.events.impl.PostPlayerUpdateEvent;
...
public class Timer extends Module {
    ...
    public void onUpdate() {
        switch (mode.getValue()) {
            case Normal -> ThunderAurora.TICK_TIMER = speed.getValue();
            case Matrix -> {
                ...
                ThunderAurora.TICK_TIMER = Math.max(speed.getValue(), 1f);
            ...
            case Grim -> {
                if (energy <= 0 || !isKeyPressed(boostKey) || ThunderAurora.core.getSetBackTime() < 2000) {
                    ThunderAurora.TICK_TIMER = 1f;
                    return;
                }
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
        if (mode.is(Mode.Grim)) {
            if (e.getPacket() instanceof CommonPingS2CPacket
                    && ThunderAurora.core.getSetBackTime() > 2000) {
