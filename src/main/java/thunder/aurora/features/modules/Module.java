package thunder.aurora.features.modules;
...
import thunder.aurora.core.manager.client.CommandManager;
...
import thunder.aurora.features.modules.client.Windows;
import thunder.aurora.features.modules.client.UnHook;
...
import static thunder.aurora.ThunderAurora.LOGGER;
import static thunder.aurora.core.Managers.NOTIFICATION;
...
public abstract class Module {
    ...
    protected void sendPacketSilent(Packet<?> packet) {
        if (mc.getNetworkHandler() == null) return;
        ThunderAurora.core.silentPackets.add(packet);
        mc.getNetworkHandler().sendPacket(packet);
    }
    ...
    public void enable() {
        ...
        if (isOn()) ThunderAurora.EVENT_BUS.subscribe(this);
        if (fullNullCheck()) return;

        LogUtils.getLogger().info("[ThunderAurora] enabled " + this.getName());
    ...
    public void disable() {
        try {
            ThunderAurora.EVENT_BUS.unsubscribe(this);
        ...
        LOGGER.info("[ThunderAurora] disabled {}", getName());
