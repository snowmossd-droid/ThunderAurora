...
import thunder.aurora.core.manager.client.MacroManager;
...
import static thunder.aurora.features.modules.Module.fullNullCheck;
...
public final class Core {
    ...
    public void onTick(PlayerUpdateEvent event) {
        ...
        for (PlayerEntity p : mc.world.getPlayers()) {
            if (p.isDead() || p.getHealth() == 0)
                ThunderAurora.EVENT_BUS.post(new EventDeath(p));
        }
    ...
    public void drawGps(DrawContext e) {
        if (ThunderAurora.gps_position != null) {
            float dst = getDistance(ThunderAurora.gps_position);
            float xOffset = mc.getWindow().getScaledWidth() / 2f;
            float yOffset = mc.getWindow().getScaledHeight() / 2f;
            float yaw = getRotations(new Vec2f(ThunderAurora.gps_position.getX(), ThunderAurora.gps_position.getZ())) - mc.player.getYaw();
            ...
            if (dst < 10)
                ThunderAurora.gps_position = null;
        }
    }
