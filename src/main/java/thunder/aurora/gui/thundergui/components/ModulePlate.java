package thunder.aurora.gui.thundergui.components;
...
import thunder.aurora.gui.thundergui.ThunderGui;
import thunder.aurora.setting.impl.Bind;
import thunder.aurora.utility.render.Render2DEngine;
import thunder.aurora.utility.render.animation.AnimationUtility;
    ...
    public void render(MatrixStack stack, int MouseX, int MouseY) {
        ...
        if (module.isOn()) {
            Render2DEngine.drawGradientRound(stack, posX + 1, posY, 89, 30, 4f,
                    Render2DEngine.applyOpacity(ThunderAuroraGui.onColor1.getValue().getColorObject(), getFadeFactor()),
                    Render2DEngine.applyOpacity(ThunderAuroraGui.onColor1.getValue().getColorObject(), getFadeFactor()),
                    Render2DEngine.applyOpacity(ThunderAuroraGui.onColor2.getValue().getColorObject(), getFadeFactor()),
                    Render2DEngine.applyOpacity(ThunderAuroraGui.onColor2.getValue().getColorObject(), getFadeFactor()));
        } else {
        ...
        if (listening_bind) {
            ...
            if (!holdbind) {
                Render2DEngine.drawRound(stack, posX + 6, posY + 6, 38, 8, 2, Render2DEngine.injectAlpha(ThunderAuroraGui.onColor1.getValue().getColorObject(), 170));
                ...
                Render2DEngine.drawRound(stack, posX + 6, posY + 16, 38, 8, 2, Render2DEngine.injectAlpha(ThunderAuroraGui.onColor1.getValue().getColorObject(), 170));
