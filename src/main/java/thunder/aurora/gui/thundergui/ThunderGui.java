package thunder.aurora.gui.thundergui;
...
import thunder.aurora.core.manager.client.ConfigManager;
import thunder.aurora.features.cmd.Command;
import thunder.aurora.features.modules.Module;
import thunder.aurora.features.modules.client.ThunderAuroraGui;
import thunder.aurora.gui.font.FontRenderers;
import thunder.aurora.gui.thundergui.components.*;
import thunder.aurora.setting.Setting;
import thunder.aurora.setting.impl.BooleanSettingGroup;
import thunder.aurora.setting.impl.ColorSetting;
import thunder.aurora.setting.impl.SettingGroup;
import thunder.aurora.utility.render.Render2DEngine;
import thunder.aurora.utility.render.animation.EaseOutBack;
...
import static thunder.aurora.utility.render.animation.AnimationUtility.fast;
...
public class ThunderGui extends Screen {
    ...
    public void renderGui(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        Render2DEngine.drawRound(context.getMatrices(), main_posX, main_posY, main_width, main_height, 9f, ThunderAuroraGui.getColorByTheme(0));

        // Плита с лого / Main GUI logo
        Render2DEngine.drawRound(context.getMatrices(), main_posX + 5, main_posY + 5, 90, 30, 7f, ThunderAuroraGui.getColorByTheme(1));

        context.getMatrices().push();
        context.getMatrices().scale(0.85f, 0.85f, 1);
        context.getMatrices().translate((main_posX + 10) / 0.85, (main_posY + 15) / 0.85, 0);
        FontRenderers.thglitch.drawString(context.getMatrices(), "THUNDERHACK", 0, 0, ThunderAuroraGui.getColorByTheme(2).getRGB());
        context.getMatrices().translate(-(main_posX + 10) / 0.85, -(main_posY + 15) / 0.85, 0);
        context.getMatrices().scale(1, 1, 1);
        context.getMatrices().pop();

        FontRenderers.settings.drawString(context.getMatrices(), "recode v" + ThunderAurora.VERSION, main_posX + 91 - (FontRenderers.settings.getStringWidth("recode v" + ThunderAurora.VERSION)), main_posY + 30, ThunderAuroraGui.getColorByTheme(3).getRGB());

        // Левая плита под категриями
        Render2DEngine.drawRound(context.getMatrices(), main_posX + 5, main_posY + 40, 90, 120, 7f, ThunderAuroraGui.getColorByTheme(4));

        // Выбор между CfgManager и FriendManager
        if (currentMode == CurrentMode.Modules) {
            Render2DEngine.drawRound(context.getMatrices(), main_posX + 20, main_posY + 195, 60, 20, 4f, ThunderAuroraGui.getColorByTheme(4));
        } else if (currentMode == CurrentMode.CfgManager) {
            Render2DEngine.drawGradientRound(context.getMatrices(), main_posX + 20, main_posY + 195, 60, 20, 4f, ThunderAuroraGui.getColorByTheme(4), ThunderAuroraGui.getColorByTheme(4), ThunderAuroraGui.getColorByTheme(5), ThunderAuroraGui.getColorByTheme(5));
        } else {
            Render2DEngine.drawGradientRound(context.getMatrices(), main_posX + 20, main_posY + 195, 60, 20, 4f, ThunderAuroraGui.getColorByTheme(5), ThunderAuroraGui.getColorByTheme(5), ThunderAuroraGui.getColorByTheme(4), ThunderAuroraGui.getColorByTheme(4));
        }

        Render2DEngine.drawRound(context.getMatrices(), main_posX + 49.5f, main_posY + 197, 1, 16, 0.5f, ThunderAuroraGui.getColorByTheme(6));

        FontRenderers.mid_icons.drawString(context.getMatrices(), "u", main_posX + 20, main_posY + 196, currentMode == CurrentMode.CfgManager ? ThunderAuroraGui.getColorByTheme(2).getRGB() : new Color(0x8D8D8D).getRGB());
        FontRenderers.mid_icons.drawString(context.getMatrices(), "v", main_posX + 54, main_posY + 197, currentMode == CurrentMode.FriendManager ? ThunderAuroraGui.getColorByTheme(2).getRGB() : new Color(0x8D8D8D).getRGB());
        ...
        if (first_open) {
            category_animation = 1;
            Render2DEngine.drawRound(context.getMatrices(), (float) (main_posX + 8), (float) CategoryY + slider_y, 84, 15, 2f, ThunderAuroraGui.getColorByTheme(7));
            first_open = false;
        } else {
            if (currentMode == CurrentMode.Modules)
                Render2DEngine.drawRound(context.getMatrices(), (float) (main_posX + 8), (float) (Render2DEngine.interpolate(CategoryY, prevCategoryY, category_animation)) + slider_y, 84, 15, 2f, ThunderAuroraGui.getColorByTheme(7));
        }
        ...
        if (currentMode != CurrentMode.Modules) {
            ...
            Render2DEngine.drawRound(context.getMatrices(), main_posX + 100, (float) main_posY + 40, (float) 295, (float) main_height - 44, 4f, ThunderAuroraGui.getColorByTheme(7));
            ...
            Render2DEngine.draw2DGradientRect(context.getMatrices(), main_posX + 102, main_posY + 34, main_posX + 393, main_posY + 60, new Color(25, 20, 30, 0), ThunderAuroraGui.getColorByTheme(7), new Color(25, 20, 30, 0), new Color(37, 27, 41, 245));
            Render2DEngine.draw2DGradientRect(context.getMatrices(), main_posX + 102, main_posY + main_height - 35, main_posX + 393, main_posY + main_height, ThunderAuroraGui.getColorByTheme(7), new Color(25, 20, 30, 0), ThunderAuroraGui.getColorByTheme(7), new Color(37, 27, 41, 0));
            Render2DEngine.popWindow();
        }
        ...
        if (currentMode == CurrentMode.CfgManager) {
            ...
            FontRenderers.modules.drawCenteredString(context.getMatrices(), "+", main_posX + 378, main_posY + 16, ThunderAuroraGui.getColorByTheme(2).getRGB());
        }
        if (currentMode == CurrentMode.FriendManager) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int clickedButton) {
        ...
        if (isHoveringItem(main_posX + 105, main_posY + 14, 11, 11, (float) mouseX, (float) mouseY)) {
            try {
                net.minecraft.util.Util.getOperatingSystem().open(new File("ThunderAurora/configs/").toURI());
            } catch (Exception e) {
        ...
        if (isHoveringItem(main_posX + 250, main_posY + 15, 140, 10, (float) mouseX, (float) mouseY) && currentMode == CurrentMode.Modules) {
            searching = true;
            ThunderAurora.currentKeyListener = ThunderAurora.KeyListening.ThunderGui;
        }
    ...
    public void keyTyped(String typedChar, int keyCode) throws IOException {
        if (ThunderAurora.currentKeyListener != ThunderAurora.KeyListening.Sliders && ThunderAurora.currentKeyListener != ThunderAurora.KeyListening.ThunderGui)
            return;
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        ...
        if (scroll_lock) {
            ...
            if (isHoveringItem(main_posX + 200, main_posY + 40, main_posX + 395, main_posY - 5 + main_height, (float) mouseX, (float) mouseY))
                scroll += dWheel * ThunderAuroraGui.scrollSpeed.getValue();
            else {
                components.forEach(component -> component.scrollElement(dWheel * ThunderAuroraGui.scrollSpeed.getValue()));
            }
            configs.forEach(component -> component.scrollElement(dWheel * ThunderAuroraGui.scrollSpeed.getValue()));
            friends.forEach(component -> component.scrollElement(dWheel * ThunderAuroraGui.scrollSpeed.getValue()));
        }
