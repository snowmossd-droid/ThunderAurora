...
public class ItemSelectWindow extends WindowBase {
    private static final Color SEARCH_BAR_HOVER = new Color(0xC5838383, true);
    private static final Color SEARCH_BAR_IDLE  = new Color(0xC5575757, true);
    private static final Color ROW_HOVER        = new Color(0xC57A7A7A, true);
    ...
    public void render(DrawContext context, int mouseX, int mouseY) {
        ...
        Render2DEngine.drawRect(context.getMatrices(), getX() + getWidth() - 90, getY() + 3, 70, 10, hover1 ? SEARCH_BAR_HOVER : SEARCH_BAR_IDLE);
        ...
        int tabColor1 = allTab ? WIN_TEXT_BRIGHT.getRGB() : Color.GRAY.getRGB();
        int tabColor2 = allTab ? Color.GRAY.getRGB() : WIN_TEXT.getRGB();
        ...
        if (!allTab && itemPlates.isEmpty()) {
            FontRenderers.sf_medium.drawCenteredString(context.getMatrices(), isRu() ? "Тут пока пусто" : "It's empty here yet",
                    getX() + getWidth() / 2f, getY() + getHeight() / 2f, WIN_TEXT.getRGB());
        }
        ...
        for (ItemPlate itemPlate : (allTab ? allItems : itemPlates)) {
            ...
            FontRenderers.sf_medium.drawString(context.getMatrices(), I18n.translate(itemPlate.key()), getX() + 26, itemPlate.offset + getY() + 38 + getScrollOffset(), WIN_TEXT.getRGB());
            ...
            Render2DEngine.drawRect(context.getMatrices(), getX() + getWidth() - 20, itemPlate.offset + getY() + 35 + getScrollOffset(), 11, 11,
                    hover2 ? ROW_HOVER : SEARCH_BAR_IDLE);
