...
public class Category extends AbstractCategory {
    ...
    public void render(DrawContext ctx, int mx, int my, float delta) {
        ...
        if (ModuleManager.clickGui.scrollMode.getValue() == ClickGui.scrollModeEn.Old
            ...
            targetH = (float) getButtonsHeight();
        else
        ...
        if (isOpen()) {
            ...
            for (AbstractButton btn : buttons) {
                if (btn instanceof ModuleButton mb && SearchBar.listening
                        && !mb.module.getName().toLowerCase().contains(SearchBar.moduleName.toLowerCase()))
                    continue;
                if (pop && buttons.getFirst().getY() + moduleOffset < getY() + height) {
                    btn.setY(getY() + height + moduleOffset);
                } else {
                    btn.setY(getY() + height);
                    moduleOffset = 0f;
                }
                btn.setX(getX());
                btn.setWidth(width);
                btn.setHeight(ModuleManager.clickGui.moduleHeight.getValue());
