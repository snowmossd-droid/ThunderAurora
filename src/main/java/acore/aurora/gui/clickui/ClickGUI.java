...
public class ClickGUI extends Screen {
    ...
    private void renderPanel(DrawContext ctx, int x, int y, Module.Category cat, int mx, int my) {
        ...
        Render2DEngine.drawRound(ctx.getMatrices(), x, y, PANEL_WIDTH, PANEL_HEIGHT, PANEL_RADIUS, guiColor);
    ...
    private Color getGuiColor() {
        try {
            AcoreAuroraGui gui = (AcoreAuroraGui) Managers.MODULE.modules.stream()
                .filter(m -> m instanceof AcoreAuroraGui).findFirst().orElse(null);
            if (gui != null) {
                return new Color(
                    AcoreAuroraGui.onColor1.getValue().getRed(),
                    AcoreAuroraGui.onColor1.getValue().getGreen(),
                    AcoreAuroraGui.onColor1.getValue().getBlue(),
                    235);
            }
        } catch (Exception ignored) {}
