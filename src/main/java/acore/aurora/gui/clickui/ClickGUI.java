...
public class ClickGUI extends Screen {
    ...
    private static final Color PANEL_BG   = new Color(18, 18, 22, 230);
    private static final Color HEADER_BG  = new Color(25, 23, 35, 245);
    private static final Color SEPARATOR  = new Color(60, 55, 80, 160);

    private static Color themeColor(int index) {
    ...
    private void renderPanel(DrawContext ctx, int x, int y, Module.Category cat, int mx, int my) {
        int titleH = TITLE_MARGIN_TOP + TITLE_HEIGHT;

        Color c_tl = themeColor(270);
        Color c_tr = themeColor(0);
        Color c_bl = themeColor(180);
        Color c_br = themeColor(90);
        Color c_tl_a = new Color(c_tl.getRed(), c_tl.getGreen(), c_tl.getBlue(), 180);
        Color c_tr_a = new Color(c_tr.getRed(), c_tr.getGreen(), c_tr.getBlue(), 180);
        Color c_bl_a = new Color(c_bl.getRed(), c_bl.getGreen(), c_bl.getBlue(), 180);
        Color c_br_a = new Color(c_br.getRed(), c_br.getGreen(), c_br.getBlue(), 180);

        Render2DEngine.drawRound(ctx.getMatrices(), x, y, PANEL_WIDTH, PANEL_HEIGHT, 12f, PANEL_BG);
        Render2DEngine.drawGradientRound(ctx.getMatrices(), x - 0.6f, y - 0.6f, PANEL_WIDTH + 1.2f, (float)titleH + 0.6f, 12f, c_tl_a, c_tr_a, c_bl_a, c_br_a);
        Render2DEngine.drawRound(ctx.getMatrices(), x, y, PANEL_WIDTH, (float)titleH, 12f, HEADER_BG);
        Render2DEngine.drawRect(ctx.getMatrices(), x + 5, y + titleH - 1f, PANEL_WIDTH - 10, 1f, SEPARATOR);
        Render2DEngine.drawRoundBorder(ctx.getMatrices(), x, y, PANEL_WIDTH, PANEL_HEIGHT, 12f, 0.6f,
            new Color(c_tl.getRed(), c_tl.getGreen(), c_tl.getBlue(), 60));

        String title = cat.getName().toUpperCase();
        float tw = FontRenderers.sf_bold.getStringWidth(title);
        FontRenderers.sf_bold.drawGradientString(ctx.getMatrices(), title,
            x + (PANEL_WIDTH - tw) / 2f, y + TITLE_MARGIN_TOP + 2, 0);
        ...
        for (Module mod : Managers.MODULE.getModulesByCategory(cat)) {
            ...
            ClickGui gui = Managers.MODULE.getModule(ClickGui.class);
            int alphaVal = gui != null ? gui.alphaModules.getValue() : 20;
            ...
            if (gui != null && gui.filling.getValue()) {
                Render2DEngine.drawGradientRound(ctx.getMatrices(),
                    x + 4, curY - 1, PANEL_WIDTH - 8, totalH - 1, (float) gui.rounding.getValue(),
                    new Color(cm2,true), new Color(cm2,true), new Color(cm2,true), new Color(cm2,true));
            }
            if (gui != null && gui.strike.getValue()) {
                Render2DEngine.drawRoundBorder(ctx.getMatrices(),
                    x + 4, curY - 1, PANEL_WIDTH - 8, totalH - 1, (float) gui.rounding.getValue(), 0.5f, new Color(cm2,true));
            }
    ...
    private void renderSearchField(DrawContext ctx) {
        ...
        Render2DEngine.drawRound(ctx.getMatrices(), sx, sy, sw, SEARCH_HEIGHT, 6f, HEADER_BG);
    ...
    private void renderThemeButton(DrawContext ctx, int mx, int my) {
        ...
        Color guiColor = getGuiColor();
        Render2DEngine.drawRound(ctx.getMatrices(), bx, by, bw, bh, 2f, hov ? HEADER_BG.brighter() : HEADER_BG);
