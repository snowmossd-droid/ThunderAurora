...
public class ClickGUI extends Screen {
    ...
    private void renderThemeButton(DrawContext ctx, int mx, int my) {
        ...
        Render2DEngine.drawTexturedRect(ctx.getMatrices(),
            net.minecraft.util.Identifier.of("acoreaurora","textures/gui/elements/colors2.png"),
            bx + 3, by + 2, 10, 10, Color.WHITE.getRGB());
    }
    ...
    private void renderColorPicker(DrawContext ctx, int x, int y, int mx, int my) {
        ...
        int cx = pw - 10, cy = 0;
        Render2DEngine.drawRound(ctx.getMatrices(), cx, cy, 10, 10, new Vector4f(0,4,0,4), new Color(ColorUtil.applyAlpha(Color.WHITE.getRGB(), alpha), true));
    ...
    public boolean mouseClicked(double mx, double my, int btn) {
        ...
        if (colorPickerOpen) {
            ...
            if (mx >= addX && mx <= addX + addW && my >= addY && my <= addY + addH) {
                ...
                ThemeManager.INSTANCE.setTheme(ThemeManager.INSTANCE.getStyles().stream().filter(s -> s.name.equals(cand)).findFirst().orElse(null));
    ...
    private Color getGuiColor() {
        try {
            AcoreAuroraGui gui = (AcoreAuroraGui) Managers.MODULE.getModules().stream()
                .filter(m -> m instanceof AcoreAuroraGui).findFirst().orElse(null);
            if (gui != null) return gui.getGuiColor();
        } catch (Exception ignored) {}
