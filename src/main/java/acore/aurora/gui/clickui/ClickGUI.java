...
public class ClickGUI extends Screen {
    ...
    private void renderPanel(DrawContext ctx, int x, int y, Module.Category cat, int mx, int my) {
        ...
        if (!icon.isEmpty()) {
            float iconH = FontRenderers.icons.getStringHeight(icon);
            FontRenderers.icons.drawString(ctx.getMatrices(), icon, startX,
                y + TITLE_MARGIN_TOP + 2 + (FONT_TITLE_BOX - iconH) / 2f, Color.WHITE.getRGB());
        }
