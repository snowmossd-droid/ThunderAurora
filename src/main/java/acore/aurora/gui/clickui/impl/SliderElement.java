...
public class SliderElement extends AbstractElement {
    ...
    private static final Color TRACK_BG = new Color(0x28FFFFFF, true);
    private static final Color KNOB_COL = new Color(0xFFE1E1E1);
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (!dragging) {
            FontRenderers.sf_medium_mini.drawString(matrixStack, setting.getName(), (setting.group != null ? 2f : 0f) + x + 6, y + 4, 0xFFC8C8D4);
            FontRenderers.sf_medium_mini.drawString(matrixStack, listening ? (Objects.equals(Stringnumber, "") ? "..." : Stringnumber) : setting.getValue() + "",
                    (int) (x + width - 6 - FontRenderers.sf_medium_mini.getStringWidth(listening ? (Objects.equals(Stringnumber, "") ? "..." : Stringnumber) : setting.getValue() + "")), y + 5, 0xFFC8C8D4);
        } else {
            if (animation > 0.2f)
                FontRenderers.sf_medium_mini.drawString(matrixStack, setting.getMin() + "", x + 6, y + 4, 0xFFC8C8D4);
            if (animation < 0.8f)
                FontRenderers.sf_medium_mini.drawString(matrixStack, setting.getMax() + "", x + width - FontRenderers.sf_medium_mini.getStringWidth(setting.getMax() + "") - 6, y + 4, 0xFFC8C8D4);
            FontRenderers.sf_medium_mini.drawString(matrixStack, listening ? (Objects.equals(Stringnumber, "") ? "..." : Stringnumber) : setting.getValue() + "", animation > 0.2f ? animation < 0.8f ? x + 6 + (width - 14) * animation - FontRenderers.sf_medium_mini.getStringWidth(setting.getValue() + "") / 2f : x + width - FontRenderers.sf_medium_mini.getStringWidth(setting.getMax() + "") - 6 : x + 6, y + 4, 0xFFC8C8D4);
        }
        Render2DEngine.drawRect(matrixStack, x + 6, y + height - 6, width - 12, 2, TRACK_BG);
        ...
        Render2DEngine.drawRect(matrixStack, (x + 6 + (width - 14) * animation), y + height - 7.5f, 2, 5, KNOB_COL);
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (listening)
            AcoreAurora.currentKeyListener = AcoreAurora.KeyListening.Sliders;
    ...
    public void keyTyped(int keyCode) {
        if (AcoreAurora.currentKeyListener != AcoreAurora.KeyListening.Sliders)
            return;
        if (listening) {
            switch (keyCode) {
