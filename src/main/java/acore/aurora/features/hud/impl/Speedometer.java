...
public class Speedometer extends HudElement {
    ...
    public void onRender2D(DrawContext context) {
        ...
        if (!bps.getValue()) {
            str += MathUtility.round(getSpeedKpH() * AcoreAurora.TICK_TIMER) + " km/h";
        } else {
            str += MathUtility.round(getSpeedMpS() * AcoreAurora.TICK_TIMER) + " b/s";
        }
        ...
        if (HudEditor.hudStyle.is(HudEditor.HudStyle.Blurry)) {
            Render2DEngine.drawRoundedBlur(context.getMatrices(), pX, getPosY(), FontRenderers.getModulesRenderer().getStringWidth(str) + 21, 13f, 3, new java.awt.Color(0,14,37,255));
