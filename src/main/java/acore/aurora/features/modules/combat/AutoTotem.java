...
public final class AutoTotem extends Module {
    ...
    public void onSync(EventSync e) {
        ...
        if (totemLegit.getValue().isEnabled()) {
            ...
            if (!legitTimer.hasTimeElapsed(d)) return;
        }
    ...
    public void swapTo(int slot) {
        if (slot != -1 && delay <= 0) {
            ...
            if (ncpBypass.getValue()) sendPacket(new PlayerMoveC2SPacket.OnGroundAndSpeedOnly(mc.player.isOnGround(), mc.player.horizontalCollision));
