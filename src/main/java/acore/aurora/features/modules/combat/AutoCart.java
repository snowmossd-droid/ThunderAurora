...
import acore.aurora.core.manager.client.AsyncManager;
...
import acore.aurora.events.impl.TotemPopEvent;
...
public class AutoCart extends Module {
   ...
   private final Setting<Float> maxDistance = new Setting<>("MaxRange", 4.5F, 2.0F, 6.0F);
   private final Setting<Bind> activeBind = new Setting<>("Keybind", new Bind(-1, false, false));
   private final Setting<Integer> cartAuraDelay = new Setting<>("AuraDelay", 0, 0, 20);
   private final Setting<Integer> refillSlot = new Setting<>("RefillSlot", 9, 1, 9);
   ...
   private final Setting<Boolean> cartAura = new Setting<>("CartAura", false);
   private final Setting<Boolean> bowAura = new Setting<>("BowAura", false);
   private final Setting<Boolean> killTarget = new Setting<>("KillTarget", true);
   private final Setting<Boolean> nearbyPlayers = new Setting<>("NearbyPlayers", false);

   private final Setting<SettingGroup> bowSettings = new Setting<>("BowSettings", new SettingGroup(false, 0), v -> mode.is(AutoCart.Mode.Bow));
   private final Setting<Integer> startDelay  = new Setting<>("BowDelay",     25, 0, 60  , v -> mode.is(AutoCart.Mode.Bow)).addToGroup(bowSettings);
   private final Setting<Integer> drawDelay   = new Setting<>("DrawDelay",   600, 100, 2000, v -> mode.is(AutoCart.Mode.Bow)).addToGroup(bowSettings);
   private final Setting<Integer> releaseDelay= new Setting<>("ReleaseDelay",100, 0, 500 , v -> mode.is(AutoCart.Mode.Bow)).addToGroup(bowSettings);
   private final Setting<Integer> delay       = new Setting<>("FireDelay",    25, 0, 100 , v -> mode.is(AutoCart.Mode.Bow)).addToGroup(bowSettings);
   private final Setting<Boolean> bowPredict  = new Setting<>("Predict",     false       , v -> mode.is(AutoCart.Mode.Bow)).addToGroup(bowSettings);
   private final Setting<Float>   bowForce    = new Setting<>("Force",       1f, 0.1f, 1f, v -> mode.is(AutoCart.Mode.Bow)).addToGroup(bowSettings);

   private final Setting<SettingGroup> crossbowSettings = new Setting<>("CrossbowSettings", new SettingGroup(false, 0), v -> mode.is(AutoCart.Mode.CrossBow));
   private final Setting<Integer> cbDelay     = new Setting<>("CBDelay",      50, 0, 500  , v -> mode.is(AutoCart.Mode.CrossBow)).addToGroup(crossbowSettings);
   private final Setting<Integer> cbLoadDelay = new Setting<>("LoadDelay",   600, 100, 3000, v -> mode.is(AutoCart.Mode.CrossBow)).addToGroup(crossbowSettings);
   private final Setting<Boolean> cbAutoLoad  = new Setting<>("AutoLoad",    true          , v -> mode.is(AutoCart.Mode.CrossBow)).addToGroup(crossbowSettings);
   private final Setting<Boolean> cbPredict   = new Setting<>("CBPredict",   false         , v -> mode.is(AutoCart.Mode.CrossBow)).addToGroup(crossbowSettings);
   ...
   private void handleRefill() {
      if (!this.scheduledRefillSwap && mc.currentScreen == null && mc.player.age == mc.player.hurtTime) {
   ...
   public void onTotemPop(@NotNull TotemPopEvent event) {
      if (!fullNullCheck() && !this.cartAuraExecuting) {
         ...
         if (popTarget != mc.player) {
            ...
            boolean isNearbyPlayer = this.nearbyPlayers.getValue()
               && !acore.aurora.core.Managers.FRIEND.isFriend(popTarget)
   ...
   private void executeBowAura(@NotNull Entity target) {
      if (!fullNullCheck() && this.mode.is(AutoCart.Mode.Bow)) {
         SearchInvResult bowResult = InventoryUtility.findItemInHotBar(Items.BOW);
         SearchInvResult cartResult = InventoryUtility.findItemInHotBar(Items.TNT_MINECART);
         if (!bowResult.found() || !cartResult.found()) return;
         BlockPos placePos = this.findCartAuraPosition(target);
         if (placePos == null) return;
         BlockPos basePos = this.getCartBasePos(placePos);
         ...
         int fireDelayMs = this.delay.getValue();
         int drawDelayMs = this.drawDelay.getValue();
         int releaseDelayMs = this.releaseDelay.getValue();
         ...
         Managers.ASYNC.run(() -> {
            try {
               ...
               if (!fullNullCheck() && !this.isDisabled() && this.mode.is(AutoCart.Mode.Bow) && this.bowAura.getValue()) {
                  ...
                  mc.execute(() -> {
                     this.applyRotation(InteractionUtility.calculateAngle(placeVec));
                     this.selectHotbarLegit(bowResult.slot());
                     mc.options.useKey.setPressed(true);
                  });

                  AsyncManager.sleep(drawDelayMs);

                  mc.execute(() -> mc.options.useKey.setPressed(false));

                  AsyncManager.sleep(releaseDelayMs);
                  ...
                  if (!finalRailExists) {
                     ...
                     AsyncManager.sleep(fireDelayMs);
                  }
