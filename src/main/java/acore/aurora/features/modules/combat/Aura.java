...
public class Aura extends Module {
    ...
    public final Setting<ESP> esp = new Setting<>("ESP", ESP.CometRing);
    ...
    public final Setting<SettingGroup> cometGroup    = new Setting<>("CometSettings",  new SettingGroup(false, 0), v -> esp.is(ESP.CometRing));
    public final Setting<Integer> cometCount         = new Setting<>("CometCount",      3, 1, 5,    v -> esp.is(ESP.CometRing)).addToGroup(cometGroup);
    public final Setting<Float>   cometRadius        = new Setting<>("CometRadius",     0.7f, 0.3f, 2.0f, v -> esp.is(ESP.CometRing)).addToGroup(cometGroup);
    public final Setting<Float>   cometSpeed         = new Setting<>("CometSpeed",      3.0f, 0.5f, 10f,  v -> esp.is(ESP.CometRing)).addToGroup(cometGroup);
    public final Setting<Integer> cometTailLength    = new Setting<>("CometTail",       18,   5,   40,    v -> esp.is(ESP.CometRing)).addToGroup(cometGroup);
    public final Setting<Float>   cometTargetHeight  = new Setting<>("CometHeight",     1.8f, 0.5f, 3.0f, v -> esp.is(ESP.CometRing)).addToGroup(cometGroup);
    public final Setting<Float>   cometSize          = new Setting<>("CometSize",       0.06f,0.02f,0.2f, v -> esp.is(ESP.CometRing)).addToGroup(cometGroup);
    public final Setting<Boolean> cometRainbow       = new Setting<>("CometRainbow",    true,             v -> esp.is(ESP.CometRing)).addToGroup(cometGroup);
    ...
    // ── Extra bypass ──
    public final Setting<Boolean> antiBotStrict  = new Setting<>("AntiBotStrict",  false, v -> antiBot.getValue()).addToGroup(bypass);
    public final Setting<Boolean> rotationNoise  = new Setting<>("RotationNoise",  false).addToGroup(bypass);
    public final Setting<Float>   noiseAmount    = new Setting<>("NoiseAmount",    0.08f, 0f, 0.5f, v -> rotationNoise.getValue()).addToGroup(bypass);
    public final Setting<Boolean> silentInteract = new Setting<>("SilentInteract", false).addToGroup(bypass);
    public final Setting<Boolean> clickDelay     = new Setting<>("ClickDelay",     false).addToGroup(bypass);
    public final Setting<Integer> clickDelayMin  = new Setting<>("ClickDelayMin",  40, 0, 150, v -> clickDelay.getValue()).addToGroup(bypass);
    public final Setting<Integer> clickDelayMax  = new Setting<>("ClickDelayMax",  80, 0, 300, v -> clickDelay.getValue()).addToGroup(bypass);
    public final Setting<Boolean> legitSwing     = new Setting<>("LegitSwing",     false).addToGroup(bypass);
    public final Setting<Boolean> groundSpoof    = new Setting<>("GroundSpoof",    false).addToGroup(bypass);
    public final Setting<Boolean> inventorySpoof = new Setting<>("InventorySpoof", false).addToGroup(bypass);
    ...
    public final Setting<Boolean> ignoreSpectator = new Setting<>("IgnoreSpectator", true).addToGroup(targets);
    public final Setting<Boolean> ignoreAdventure = new Setting<>("IgnoreAdventure", false).addToGroup(targets);
    ...
    private void swingHand() {
        if (legitSwing.getValue()) {
            // LegitSwing: gửi packet swing thay vì swing trực tiếp để bypass một số AC
            sendPacket(new HandSwingC2SPacket(attackHand.getValue() == AttackHand.OffHand ? Hand.OFF_HAND : Hand.MAIN_HAND));
            return;
        }
    ...
    private void calcRotations(boolean ready) {
        ...
        if (trackticks > 0 || rotationMode.getValue() == Mode.Track) {
            ...
            // RotationNoise bypass — thêm nhiễu nhỏ để bypass AC phát hiện perfect rotation
            if (rotationNoise.getValue()) {
                float n = noiseAmount.getValue();
                rotationYaw   += (float)(Math.random() * 2 - 1) * n;
                rotationPitch += (float)(Math.random() * 2 - 1) * n * 0.5f;
            }
        } else {
            rotationYaw = mc.player.getYaw();
            rotationPitch = mc.player.getPitch();
    ...
    public void onRender3D(MatrixStack stack) {
        ...
        switch (esp.getValue()) {
            ...
            case CometRing   -> renderCometRingEsp(target);
        }
    ...
    public void renderCometRingEspPublic(Entity target) { renderCometRingEsp(target); }

    // ── CometRing ESP ──────────────────────────────────────────────────────────
    private double cometAngle = 0;
    @SuppressWarnings("unchecked")
    private final java.util.LinkedList<Vec3d>[] cometTrails = new java.util.LinkedList[5];

    {
        for (int i = 0; i < cometTrails.length; i++)
            cometTrails[i] = new java.util.LinkedList<>();
    }

    /**
     * 3 vòng hình tròn quay quanh target từ chân lên đầu.
     * Mỗi vòng là một comet: đầu sáng, đuôi nhọn fade dần.
     * Quỹ đạo: X/Z là circle, Y dao động sin từ feet→head tạo đường xoắn ốc.
     */
    private void renderCometRingEsp(Entity target) {
        int   count     = cometCount.getValue();
        float radius    = cometRadius.getValue();
        float speed     = cometSpeed.getValue();
        int   tailLen   = cometTailLength.getValue();
        float totalH    = cometTargetHeight.getValue();
        float headSize  = cometSize.getValue();
        boolean rainbow = cometRainbow.getValue();
        ...
        cometAngle += speed * 0.03;

        for (int i = 0; i < count; i++) {
            // Mỗi comet lệch đều 360°/count
            double angle = cometAngle + (Math.PI * 2.0 / count) * i;

            // X,Z: tròn theo radius
            double cx = tx + Math.sin(angle) * radius;
            double cz = tz + Math.cos(angle) * radius;
            // Y: dao động sin 0→totalH, tạo cảm giác quay từ chân (ty) lên đầu (ty+totalH)
            double cy = ty + totalH * 0.5 + Math.sin(angle) * (totalH * 0.5);

            // Lưu vị trí đầu comet vào trail
            java.util.LinkedList<Vec3d> trail = cometTrails[i];
            trail.addFirst(new Vec3d(cx, cy, cz));
            while (trail.size() > tailLen) trail.removeLast();

            // Màu theo theme
            Color headColor = rainbow
                ? HudEditor.getColor((int)(i * (360f / count) + System.currentTimeMillis() / 25f))
                : HudEditor.getColor(i * (360 / count));

            // Vẽ từng đoạn trail, nhỏ dần và mờ dần (đuôi nhọn)
            Vec3d[] pts = trail.toArray(new Vec3d[0]);
            for (int j = 0; j < pts.length; j++) {
                float progress = 1.0f - (float) j / tailLen; // 1.0 ở đầu → 0.0 ở đuôi
                float s = headSize * progress;               // kích thước nhỏ dần → nhọn
                int   a = (int)(220 * progress * progress);  // alpha mờ nhanh ở đuôi

                Color c = new Color(headColor.getRed(), headColor.getGreen(),
                                    headColor.getBlue(), Math.max(0, a));

                Box box = new Box(pts[j].x - s, pts[j].y - s * 1.4, pts[j].z - s,
                                  pts[j].x + s, pts[j].y + s * 1.4, pts[j].z + s);
                Render3DEngine.FILLED_QUEUE.add(new Render3DEngine.FillAction(box, c));

                // Viền nhẹ chỉ ở phần đầu (sáng nhất)
                if (j < 4) {
                    Render3DEngine.OUTLINE_QUEUE.add(
                        new Render3DEngine.OutlineAction(box,
                            new Color(headColor.getRed(), headColor.getGreen(),
                                      headColor.getBlue(), (int)(160 * progress)), 1));
                }
            }
        }
    }
    ...
    private boolean skipEntity(Entity entity) {
        ...
        if (entity instanceof PlayerEntity player) {
            ...
            if (player.isCreative() && ignoreCreative.getValue())
                return true;
            if (player.isSpectator() && ignoreSpectator.getValue())
                return true;
            if (!player.isCreative() && !player.isSpectator() && player.getAbilities().allowFlying && ignoreAdventure.getValue())
                return true;
    ...
    public enum ESP {
        Off, NurikZapen, CelkaPasta, Orbit, Spectre, Wraith, CometRing
    }
