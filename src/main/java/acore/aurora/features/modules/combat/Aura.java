...
import acore.aurora.utility.interfaces.IOtherClientPlayerEntity;
...
import acore.aurora.utility.player.PlayerUtility;
...
import static acore.aurora.utility.math.MathUtility.random;
...
public class Aura extends Module {
    ...
    public final Setting<ESP> esp = new Setting<>("ESP", ESP.AcoreAurora);
    
    public final Setting<SettingGroup> orbitGroup = new Setting<>("OrbitSettings", new SettingGroup(false, 0), v -> esp.is(ESP.Orbit));
    public final Setting<Float> orbitRadius = new Setting<>("Radius", 1.2f, 0.3f, 2.5f, v -> esp.is(ESP.Orbit)).addToGroup(orbitGroup);
    public final Setting<Float> orbitSpeed = new Setting<>("Speed", 2.0f, 0.5f, 8.0f, v -> esp.is(ESP.Orbit)).addToGroup(orbitGroup);
    public final Setting<Integer> orbitCount = new Setting<>("Beads", 8, 3, 24, v -> esp.is(ESP.Orbit)).addToGroup(orbitGroup);
    public final Setting<Float> orbitSize = new Setting<>("Size", 0.12f, 0.05f, 0.35f, v -> esp.is(ESP.Orbit)).addToGroup(orbitGroup);
    public final Setting<Boolean> orbitRainbow = new Setting<>("Rainbow", true, v -> esp.is(ESP.Orbit)).addToGroup(orbitGroup);
    public final Setting<Integer> orbitOutline = new Setting<>("Outline", 2, 0, 5, v -> esp.is(ESP.Orbit)).addToGroup(orbitGroup);
    
    public final Setting<SettingGroup> spectreGroup = new Setting<>("SpectreSettings", new SettingGroup(false, 0), v -> esp.is(ESP.Spectre));
    public final Setting<Integer> spectreLength = new Setting<>("Length", 14, 1, 40, v -> esp.is(ESP.Spectre)).addToGroup(spectreGroup);
    public final Setting<Integer> spectreFactor = new Setting<>("Factor", 8, 1, 20, v -> esp.is(ESP.Spectre)).addToGroup(spectreGroup);
    public final Setting<Float> spectreShaking = new Setting<>("Shaking", 1.8f, 1.5f, 10f, v -> esp.is(ESP.Spectre)).addToGroup(spectreGroup);
    public final Setting<Float> spectreAmplitude = new Setting<>("Amplitude", 3f, 0.1f, 8f, v -> esp.is(ESP.Spectre)).addToGroup(spectreGroup);
    
    public final Setting<SettingGroup> wraithGroup = new Setting<>("WraithSettings", new SettingGroup(false, 0), v -> esp.is(ESP.Wraith));
    ...
    public final Setting<SettingGroup> espGroup = new Setting<>("ESPSettings", new SettingGroup(false, 0), v -> esp.is(ESP.AcoreAuroraV2));
    public final Setting<Integer> espLength = new Setting<>("ESPLength", 14, 1, 40, v -> esp.is(ESP.AcoreAuroraV2)).addToGroup(espGroup);
    public final Setting<Integer> espFactor = new Setting<>("ESPFactor", 8, 1, 20, v -> esp.is(ESP.AcoreAuroraV2)).addToGroup(espGroup);
    public final Setting<Float> espShaking = new Setting<>("ESPShaking", 1.8f, 1.5f, 10f, v -> esp.is(ESP.AcoreAuroraV2)).addToGroup(espGroup);
    public final Setting<Float> espAmplitude = new Setting<>("ESPAmplitude", 3f, 0.1f, 8f, v -> esp.is(ESP.AcoreAuroraV2)).addToGroup(espGroup);

    public final Setting<SettingGroup> ghostV1Group = new Setting<>("GhostV1Settings", new SettingGroup(false, 0), v -> esp.is(ESP.GhostV1));
    public final Setting<Float> ghostV1Speed = new Setting<>("Speed", 0.62f, 0.1f, 3.0f, v -> esp.is(ESP.GhostV1)).addToGroup(ghostV1Group);

    public final Setting<SettingGroup> ghostV2Group = new Setting<>("GhostV2Settings", new SettingGroup(false, 0), v -> esp.is(ESP.GhostV2));
    public final Setting<Integer> ghostV2ParticleLimit = new Setting<>("ParticleLimit", 3, 1, 8, v -> esp.is(ESP.GhostV2)).addToGroup(ghostV2Group);
    public final Setting<Float> ghostV2ParticleSize = new Setting<>("ParticleSize", 0.28f, 0.05f, 1.0f, v -> esp.is(ESP.GhostV2)).addToGroup(ghostV2Group);
    public final Setting<Float> ghostV2Length = new Setting<>("Length", 20.5f, 5f, 60f, v -> esp.is(ESP.GhostV2)).addToGroup(ghostV2Group);
    ...
    public final Setting<Boolean> fakePacket     = new Setting<>("FakePacket",    false).addToGroup(bypass);
    ...
    public final Setting<Boolean> antiBot        = new Setting<>("AntiBot",       false).addToGroup(bypass);
    public final Setting<Boolean> grimBypass     = new Setting<>("GrimBypass",    false).addToGroup(bypass);
    public final Setting<Boolean> ncpBypass      = new Setting<>("NCPBypass",     false).addToGroup(bypass);
    public final Setting<Boolean> aacBypass      = new Setting<>("AACBypass",     false).addToGroup(bypass);
    public final Setting<Boolean> verusBypass    = new Setting<>("VerusBypass",   false).addToGroup(bypass);
    public final Setting<Boolean> karhuBypass    = new Setting<>("KarhuBypass",   false).addToGroup(bypass);
    public final Setting<Boolean> watchdogBypass = new Setting<>("WatchdogBypass",false).addToGroup(bypass);
    public final Setting<Boolean> vusBypass      = new Setting<>("VulcanBypass",  false).addToGroup(bypass);
    public final Setting<Boolean> sparkyBypass   = new Setting<>("SparkyBypass",  false).addToGroup(bypass);
    public final Setting<Boolean> noHurtTime     = new Setting<>("NoHurtTime",    false).addToGroup(bypass);
    public final Setting<Integer> hurtTimeMs     = new Setting<>("HurtTimeMs",    500, 0, 1000, v -> noHurtTime.getValue()).addToGroup(bypass);
    ...
    public final Setting<Boolean> packetSprint   = new Setting<>("PacketSprint",  false).addToGroup(bypass);
    public final Setting<Boolean> fakeDelay      = new Setting<>("FakeDelay",     false).addToGroup(bypass);
    public final Setting<Integer> fakeDelayMs    = new Setting<>("FakeMs",        50, 0, 500, v -> fakeDelay.getValue()).addToGroup(bypass);
    ...
    public final Setting<SettingGroup> ringsGroup = new Setting<>("RingsSettings", new SettingGroup(false, 0), v -> esp.is(ESP.Rings));
    public final Setting<Integer> ringsCount  = new Setting<>("RingsCount",  3, 1, 6,    v -> esp.is(ESP.Rings)).addToGroup(ringsGroup);
    public final Setting<Float>   ringsRadius = new Setting<>("RingsRadius", 0.6f, 0.2f, 2f, v -> esp.is(ESP.Rings)).addToGroup(ringsGroup);
    public final Setting<Float>   ringsSpeed  = new Setting<>("RingsSpeed",  2f, 0.5f, 10f, v -> esp.is(ESP.Rings)).addToGroup(ringsGroup);
    public final Setting<Float>   ringsHeight = new Setting<>("RingsHeight", 2f, 0.5f, 4f,  v -> esp.is(ESP.Rings)).addToGroup(ringsGroup);
    public final Setting<Boolean> ringsRainbow= new Setting<>("RingsRainbow",true,           v -> esp.is(ESP.Rings)).addToGroup(ringsGroup);
    public final Setting<Boolean> ringsParticle=new Setting<>("RingsParticle",true,          v -> esp.is(ESP.Rings)).addToGroup(ringsGroup);
    public final Setting<Float>   ringsParticleSpeed = new Setting<>("ParticleSpeed", 1f, 0.1f, 5f, v -> esp.is(ESP.Rings) && ringsParticle.getValue()).addToGroup(ringsGroup);
    ...
    public void onUpdate(PlayerUpdateEvent e) {
        ...
        if (mc.player.isUsingItem() && pauseWhileEating.getValue())
            return;
        if(pauseBaritone.getValue() && AcoreAurora.baritone){
        ...
        orbitAngle += orbitSpeed.getValue() * 0.05;
    ...
    public float getAttackCooldownProgressPerTick() {
        return (float) (1.0 / mc.player.getAttributeValue(EntityAttributes.GENERIC_ATTACK_SPEED) * (20.0 * AcoreAurora.TICK_TIMER * (tpsSync.getValue() ? Managers.SERVER.getTPSFactor() : 1f)));
    }
    ...
    public void onRender3D(MatrixStack stack) {
        ...
        switch (esp.getValue()) {
            ...
            case AcoreAuroraV2 -> Render3DEngine.renderGhosts(espLength.getValue(), espFactor.getValue(), espShaking.getValue(), espAmplitude.getValue(), target);
            case AcoreAurora -> Render3DEngine.drawTargetEsp(stack, target);
            case Orbit -> renderOrbitEsp(target);
            case Spectre -> renderSpectre(target);
            case Wraith -> renderWraith(target);
            case Rings -> renderRingsEsp(stack, target);
            case Visuals -> renderVisualsEsp(stack, target);
    ...
    private void renderOrbitEsp(Entity target) {
        ...
        int totalBeads = orbitCount.getValue();
        int outline = orbitOutline.getValue();
        ...
        for (int i = 0; i < totalBeads; i++) {
            ...
            Color color;
            if (orbitRainbow.getValue()) {
                color = HudEditor.getColor((int) (progress * 360 + System.currentTimeMillis() / 30f));
            } else {
                color = HudEditor.getColor((int) (progress * 360));
            }
            ...
            Box box = new Box(x - beadSize, cy - beadSize, z - beadSize, x + beadSize, cy + beadSize, z + beadSize);
            
            Render3DEngine.FILLED_QUEUE.add(new Render3DEngine.FillAction(box, fill));
            
            if (outline > 0) {
                Render3DEngine.OUTLINE_QUEUE.add(new Render3DEngine.OutlineAction(box, color, outline));
            }
        }
    }

    private void renderSpectre(Entity target) {
        if (target instanceof LivingEntity living) {
            Render3DEngine.renderGhosts(spectreLength.getValue(), spectreFactor.getValue(), spectreShaking.getValue(), spectreAmplitude.getValue(), living);
        }
    }
    ...
    private double ringsAngle = 0;
    private final java.util.List<Vec3d> particleTrail = new java.util.LinkedList<>();
    private Vec3d lastTargetPos = null;

    private final acore.aurora.utility.render.animation.advanced.Animation ghostV1Anim =
            new acore.aurora.utility.render.animation.advanced.Animation()
                    .setEasing(acore.aurora.utility.render.animation.advanced.Easing.EASE_OUT_QUAD)
                    .setSpeed(400)
                    .setSize(1.0F)
                    .setForward(false);
    ...
    private void renderRingsEsp(MatrixStack stack, Entity target) {
        int count = ringsCount.getValue();
        float radius = ringsRadius.getValue();
        float speed = ringsSpeed.getValue();
        float h = ringsHeight.getValue();
        boolean rainbow = ringsRainbow.getValue();

        ringsAngle += speed * 0.05;

        double td = Render3DEngine.getTickDelta();
        double tx = target.prevX + (target.getX() - target.prevX) * td;
        double ty = target.prevY + (target.getY() - target.prevY) * td;
        double tz = target.prevZ + (target.getZ() - target.prevZ) * td;

        if (ringsParticle.getValue()) {
            Vec3d cur = new Vec3d(tx, ty, tz);
            if (lastTargetPos != null && cur.distanceTo(lastTargetPos) > 0.05) {
                particleTrail.add(0, cur);
                if (particleTrail.size() > 20) particleTrail.remove(particleTrail.size() - 1);
            }
            lastTargetPos = cur;
        }

        for (int i = 0; i < count; i++) {
            float ringY = (float)(ty + (h * i / (float) count));
            Color c = rainbow ? HudEditor.getColor((int)(i * (360f / count))) : HudEditor.getColor(0);
            Entity fakeEnt = new net.minecraft.entity.decoration.ArmorStandEntity(mc.world, tx, ringY - ty + ty, tz) {
            };
            Render3DEngine.drawCircle3D(stack, target, radius + (float)(Math.sin(ringsAngle + i) * 0.1), c.getRGB(), 32, rainbow, i * 30);
        }

        Render3DEngine.drawCircle3D(stack, target, radius * 0.4f, HudEditor.getColor(180).getRGB(), 24, rainbow, 180);
    }

    private double visualsAngle = 0;
    private double visualsSwirlAngle = 0;
    ...
    private static class FallingLine {
        ...
        float life;
        float maxLife;
        FallingLine(double x, double y, double z, double speed) {
            this.x = x; this.y = y; this.z = z;
            this.speed = speed;
            this.maxLife = 1.8f + (float)(Math.random() * 1.2f);
            this.life = maxLife;
        }
        boolean update() {
            y -= speed;
    ...
    private static class VisualOrb {
        double angle;
        double height;
        float size;
        VisualOrb(double angle, double height, float size) {
            this.angle = angle; this.height = height; this.size = size;
        }
    }

    private final java.util.List<FallingLine> fallingLines = new CopyOnWriteArrayList<>();
    private final acore.aurora.utility.Timer visualLineTimer = new acore.aurora.utility.Timer();
    private final java.util.List<VisualOrb> visualOrbs = new java.util.ArrayList<>();

    private void renderVisualsEsp(MatrixStack stack, Entity target) {
        float td = Render3DEngine.getTickDelta();
        double tx = target.prevX + (target.getX() - target.prevX) * td;
        double ty = target.prevY + (target.getY() - target.prevY) * td;
        double tz = target.prevZ + (target.getZ() - target.prevZ) * td;
        ...
        visualsAngle += 0.04;
        visualsSwirlAngle += 0.07;

        if (visualOrbs.isEmpty()) {
            for (int i = 0; i < 3; i++) {
                visualOrbs.add(new VisualOrb(Math.PI * 2.0 / 3.0 * i, targetHeight * 0.5, 0.13f + (float)(Math.random() * 0.06f)));
            }
        }

        if (visualLineTimer.passedMs(120)) {
            visualLineTimer.reset();
            double spread = 1.4;
            double lx = tx + (Math.random() - 0.5) * spread * 2;
            double lz = tz + (Math.random() - 0.5) * spread * 2;
            double startY = ty + targetHeight + 2.5 + Math.random() * 2.0;
            fallingLines.add(new FallingLine(lx, startY, lz, 0.04 + Math.random() * 0.03));
        }
        fallingLines.removeIf(FallingLine::update);
        ...
        for (FallingLine line : fallingLines) {
            float alpha = Math.min(1f, line.life / line.maxLife) * 0.75f;
            Color lc = HudEditor.getColor((int)(System.currentTimeMillis() / 10f % 360));
            Color lineColor = new Color(lc.getRed(), lc.getGreen(), lc.getBlue(), (int)(alpha * 200));

            net.minecraft.client.render.BufferBuilder buf = tess.begin(
                net.minecraft.client.render.VertexFormat.DrawMode.LINES,
                net.minecraft.client.render.VertexFormats.LINES
            );
            stack.push();
            stack.translate(line.x - camX, line.y - camY, line.z - camZ);
            org.joml.Matrix4f mat = stack.peek().getPositionMatrix();
            org.joml.Vector3f normal = new org.joml.Vector3f(0f, 1f, 0f);
            buf.vertex(mat, 0f, 0f, 0f).color(lineColor.getRGB()).normal(stack.peek(), normal.x, normal.y, normal.z);
            float lineLen = 0.8f + (line.maxLife - line.life) * 0.3f;
            buf.vertex(mat, 0f, lineLen, 0f).color(new Color(lineColor.getRed(), lineColor.getGreen(), lineColor.getBlue(), 0).getRGB()).normal(stack.peek(), normal.x, normal.y, normal.z);
            Render2DEngine.endBuilding(buf);
            stack.pop();
        }

        int swirlSteps = 48;
        float swirlRadius = 0.55f;
        for (int i = 0; i < swirlSteps; i++) {
            float progress = (float) i / swirlSteps;
            double angle = visualsSwirlAngle + progress * Math.PI * 4;
            double r = swirlRadius * (0.3 + progress * 0.7);
            double sx = tx + Math.cos(angle) * r - camX;
            double sy = ty + progress * targetHeight - camY;
            double sz = tz + Math.sin(angle) * r - camZ;

            float alpha = 0.15f + progress * 0.6f;
            Color sc = HudEditor.getColor((int)(progress * 360 + System.currentTimeMillis() / 15f % 360));
            Color swirlColor = new Color(sc.getRed(), sc.getGreen(), sc.getBlue(), (int)(alpha * 220));
            ...
            stack.push();
            stack.translate(sx, sy, sz);
            org.joml.Matrix4f mat = stack.peek().getPositionMatrix();
            buf.vertex(mat, -dot, -dot, 0f).color(swirlColor.getRGB());
            buf.vertex(mat,  dot, -dot, 0f).color(swirlColor.getRGB());
            buf.vertex(mat,  dot,  dot, 0f).color(swirlColor.getRGB());
            buf.vertex(mat, -dot,  dot, 0f).color(swirlColor.getRGB());
            Render2DEngine.endBuilding(buf);
        ...
        for (int i = 0; i < visualOrbs.size(); i++) {
            VisualOrb orb = visualOrbs.get(i);
            orb.angle = visualsAngle + Math.PI * 2.0 / visualOrbs.size() * i;
            double ox = tx + Math.cos(orb.angle) * 0.7 - camX;
            double oy = ty + orb.height - camY;
            double oz = tz + Math.sin(orb.angle) * 0.7 - camZ;
            float s = orb.size;
            Color oc = HudEditor.getColor((int)(i * 120 + System.currentTimeMillis() / 20f % 360));
            Color orbColor = new Color(oc.getRed(), oc.getGreen(), oc.getBlue(), 200);
            ...
            org.joml.Matrix4f mat = stack.peek().getPositionMatrix();
            buf.vertex(mat, -s, -s, 0f).color(orbColor.getRGB());
            buf.vertex(mat,  s, -s, 0f).color(orbColor.getRGB());
            buf.vertex(mat,  s,  s, 0f).color(orbColor.getRGB());
            buf.vertex(mat, -s,  s, 0f).color(orbColor.getRGB());
            Render2DEngine.endBuilding(buf);
    ...
    private static class GhostV2EspRenderer {
        private final java.util.List<acore.aurora.utility.render.GhostRenderer3D> particles = new java.util.ArrayList<>();
        private final acore.aurora.utility.render.animation.advanced.InfinityAnimation moving =
                new acore.aurora.utility.render.animation.advanced.InfinityAnimation();
        private final acore.aurora.utility.render.animation.advanced.Animation targetEspAnim =
                new acore.aurora.utility.render.animation.advanced.Animation()
                        .setEasing(acore.aurora.utility.render.animation.advanced.Easing.TARGETESP_EASE_OUT_BACK)
                        .setSpeed(300)
                        .setSize(1.0F)
                        .setForward(false);
        ...
        void render(LivingEntity target, int particleLimit, float particleSize, float trailLength) {
            ...
            for (int index = 0; index < particles.size(); index++) {
                acore.aurora.utility.render.GhostRenderer3D particle = particles.get(index);
        ...
        private void spawnParticleIfNeeded(LivingEntity target, int particleLimit, float particleSize, float trailLength) {
            ...
            while (particles.size() < desired) {
                particles.add(new acore.aurora.utility.render.GhostRenderer3D(target.getPos(), Vec3d.ZERO, particleSize, trailLength));
            }
            while (particles.size() > desired) {
        ...
        private void updateParticle(acore.aurora.utility.render.GhostRenderer3D particle, int index, float fpsFactor, LivingEntity target, float movementValue, float animationFactor, int particleLimit, float particleSize) {
    ...
    public enum ESP {
        Off, AcoreAurora, NurikZapen, CelkaPasta, AcoreAuroraV2, Orbit, Spectre, Wraith, Rings, Visuals, GhostV1, GhostV2
    }
