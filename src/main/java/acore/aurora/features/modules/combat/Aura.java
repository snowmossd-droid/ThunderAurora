...
public class Aura extends Module {
    ...
    public final Setting<SettingGroup> ringGroup = new Setting<>("RingSettings", new SettingGroup(true, 0), v -> esp.is(ESP.Ring));
    public final Setting<Integer> ringBalls = new Setting<>("Balls", 3, 1, 8, v -> esp.is(ESP.Ring)).addToGroup(ringGroup);
    public final Setting<Float> ringSize = new Setting<>("BallSize", 0.22f, 0.05f, 1.0f, v -> esp.is(ESP.Ring)).addToGroup(ringGroup);
    public final Setting<Float> ringSpeed = new Setting<>("SpinSpeed", 4.0f, 0.5f, 15.0f, v -> esp.is(ESP.Ring)).addToGroup(ringGroup);
    public final Setting<Float> ringRadius = new Setting<>("Radius", 0.0f, -0.5f, 2.0f, v -> esp.is(ESP.Ring)).addToGroup(ringGroup);
    ...
    public void onRender3D(MatrixStack stack) {
        ...
        switch (esp.getValue()) {
            ...
            case Ring -> renderRingEsp(stack, target);
        }
    ...
    private float ringAnglePhase = 0f;
    ...
    private void renderRingEsp(MatrixStack stack, Entity target) {
        if (!(target instanceof LivingEntity living)) return;

        int balls = Math.max(1, ringBalls.getValue());
        float size = ringSize.getValue();

        Box box = living.getBoundingBox();
        double hitboxWidth = Math.max(box.getLengthX(), box.getLengthZ());
        double radius = hitboxWidth * 0.5 + size * 0.25 + ringRadius.getValue();
        double centerY = box.getLengthY() * 0.5 - 0.7;

        ringAnglePhase = (ringAnglePhase + ringSpeed.getValue()) % 360f;

        double ex = living.prevX + (living.getX() - living.prevX) * Render3DEngine.getTickDelta() - mc.gameRenderer.getCamera().getPos().getX();
        double ey = living.prevY + (living.getY() - living.prevY) * Render3DEngine.getTickDelta() - mc.gameRenderer.getCamera().getPos().getY();
        double ez = living.prevZ + (living.getZ() - living.prevZ) * Render3DEngine.getTickDelta() - mc.gameRenderer.getCamera().getPos().getZ();
        ...
        Color col = HudEditor.getColor(0);

        BufferBuilder buffer = Tessellator.getInstance().begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_TEXTURE_COLOR);

        for (int i = 0; i < balls; i++) {
            float angleOffset = i * 360.0F / balls;
            double rad = Math.toRadians(ringAnglePhase + angleOffset);
            double offsetX = Math.sin(rad) * radius;
            double offsetZ = Math.cos(rad) * radius;

            double px = ex + offsetX;
            double py = ey + centerY;
            double pz = ez + offsetZ;

            stack.push();
            stack.translate(px, py, pz);
            stack.multiply(net.minecraft.util.math.RotationAxis.POSITIVE_Y.rotationDegrees(-camYaw));
            stack.multiply(net.minecraft.util.math.RotationAxis.POSITIVE_X.rotationDegrees(camPitch));

            Matrix4f m = stack.peek().getPositionMatrix();
            float hs = size * 0.5f;

            buffer.vertex(m, -hs, -hs, 0).texture(0, 0).color(col.getRGB());
            buffer.vertex(m,  hs, -hs, 0).texture(1, 0).color(col.getRGB());
            buffer.vertex(m,  hs,  hs, 0).texture(1, 1).color(col.getRGB());
            buffer.vertex(m, -hs,  hs, 0).texture(0, 1).color(col.getRGB());
    ...
    private static class GhostV2EspRenderer {
        ...
        void render(LivingEntity target, int particleLimit, float particleSize, float trailLength) {
            ...
            for (acore.aurora.utility.render.GhostRenderer3D particle : particles) {
    ...
    public enum ESP {
        Off, NurikZapen, CelkaPasta, Wraith, GhostV1, GhostV2, Ring
    }
