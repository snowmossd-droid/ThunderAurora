...
public class Aura extends Module {
    ...
    private void renderRingsEsp(MatrixStack stack, Entity target) {
        ...
        float height = ringsHeight.getValue();
        boolean rainbow = ringsRainbow.getValue();

        ringsAngle += speed * 0.05;

        double tx = target.prevX + (target.getX() - target.prevX) * Render3DEngine.getTickDelta();
        double ty = target.prevY + (target.getY() - target.prevY) * Render3DEngine.getTickDelta();
        double tz = target.prevZ + (target.getZ() - target.prevZ) * Render3DEngine.getTickDelta();
        ...
        if (ringsParticle.getValue()) {
            ...
            int pi = 0;
            for (Vec3d p : particleTrail) {
                float alpha = 1f - (pi / (float) particleTrail.size());
                Color pc = rainbow ? HudEditor.getColor((int)(pi * 18)) : HudEditor.getColor(0);
                Color c = new Color(pc.getRed(), pc.getGreen(), pc.getBlue(), (int)(alpha * 180));
                Render3DEngine.drawPoint(stack, p.x, p.y + 0.9, p.z, 0.07f * alpha * ringsParticleSpeed.getValue(), c);
                pi++;
            }
        }

        for (int i = 0; i < count; i++) {
            float ringY = ty + (height * i / (float) count);
            double phase = ringsAngle + (Math.PI * 2 * i / count);
            Color c = rainbow ? HudEditor.getColor((int)(i * (360f / count))) : HudEditor.getColor(0);
            drawRing(stack, tx, ringY, tz, radius, phase, c);
        }

        float topRingY = (float)(ty + height);
        Color topColor = rainbow ? HudEditor.getColor(180) : HudEditor.getColor(0);
        drawRing(stack, tx, topRingY, tz, radius * 0.5f, ringsAngle * 2, topColor);
    }

    private void drawRing(MatrixStack stack, double cx, double cy, double cz, float radius, double angleOffset, Color color) {
        int segments = 32;
        com.mojang.blaze3d.systems.RenderSystem.enableBlend();
        com.mojang.blaze3d.systems.RenderSystem.defaultBlendFunc();

        var tessellator = net.minecraft.client.render.Tessellator.getInstance();
        var buffer = tessellator.begin(net.minecraft.client.render.VertexFormat.DrawMode.DEBUG_LINE_STRIP,
                net.minecraft.client.render.VertexFormats.POSITION_COLOR);

        var camPos = net.minecraft.util.math.MathHelper.lerp(Render3DEngine.getTickDelta(),
                mc.gameRenderer.getCamera().getPos().x, mc.gameRenderer.getCamera().getPos().x);
        var cam = mc.gameRenderer.getCamera().getPos();

        stack.push();
        var mat = stack.peek().getPositionMatrix();

        for (int i = 0; i <= segments; i++) {
            double angle = angleOffset + (Math.PI * 2 * i / segments);
            float x = (float)(cx - cam.x + Math.cos(angle) * radius);
            float y = (float)(cy - cam.y);
            float z = (float)(cz - cam.z + Math.sin(angle) * radius);
            buffer.vertex(mat, x, y, z).color(color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha());
        }

        net.minecraft.client.gl.VertexBuffer vb = null;
        try {
            var built = buffer.end();
            net.minecraft.client.render.RenderSystem.setShader(net.minecraft.client.render.GameRenderer::getPositionColorProgram);
            tessellator.draw();
