...
public class Aura extends Module {
    ...
    private void calcRotations(boolean ready) {
        ...
        if (rotationMode.is(Mode.MLSAC)) {
            ...
            float noiseYaw   = (float)(Math.sin(mlsacNoisePhase * 0.13 + Math.cos(mlsacNoisePhase * 0.07)) * 0.55
                                     + Math.sin(mlsacNoisePhase * 0.031) * 0.18);
            float noisePitch = (float)(Math.cos(mlsacNoisePhase * 0.11 + Math.sin(mlsacNoisePhase * 0.05)) * 0.35
                                     + Math.cos(mlsacNoisePhase * 0.047) * 0.12);

            float springK   = 0.18f;
            float dampening = 0.72f;
            ...
            float maxV = random(minYawStep.getValue() * 0.6f, maxYawStep.getValue() * 0.8f);
            mlsacVelocityYaw   = MathHelper.clamp(mlsacVelocityYaw,   -maxV, maxV);
            mlsacVelocityPitch = MathHelper.clamp(mlsacVelocityPitch, -maxV * 0.5f, maxV * 0.5f);
