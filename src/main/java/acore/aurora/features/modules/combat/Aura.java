...
public class Aura extends Module {
    ...
    private float mlsacNoiseYaw      = 0f;
    private float mlsacNoisePitch    = 0f;
    private final java.util.Random mlsacRng = new java.util.Random();
    ...
    private void calcRotations(boolean ready) {
        ...
        if (rotationMode.is(Mode.MLSAC)) {
            ...
            // Brownian noise: Gaussian random walk with mean-reversion
            // variance không đều → jerk distribution giống human hơn sinusoidal
            float noiseDrift = 0.82f;
            mlsacNoiseYaw   = mlsacNoiseYaw   * noiseDrift + (float)(mlsacRng.nextGaussian() * 0.24);
            mlsacNoisePitch = mlsacNoisePitch * noiseDrift + (float)(mlsacRng.nextGaussian() * 0.15);
            mlsacNoiseYaw   = MathHelper.clamp(mlsacNoiseYaw,   -1.4f, 1.4f);
            mlsacNoisePitch = MathHelper.clamp(mlsacNoisePitch, -0.8f, 0.8f);

            // Spring constant vary mỗi tick → acceleration không đều, jerk variance cao hơn
            float springK   = 0.15f + (float)(mlsacRng.nextGaussian() * 0.022f);
            float dampening = 0.70f + (float)(mlsacRng.nextGaussian() * 0.018f);
            springK   = MathHelper.clamp(springK,   0.08f, 0.26f);
            dampening = MathHelper.clamp(dampening, 0.58f, 0.82f);

            mlsacVelocityYaw   = mlsacVelocityYaw   * dampening + delta_yaw   * springK + mlsacNoiseYaw;
            mlsacVelocityPitch = mlsacVelocityPitch * dampening + delta_pitch * springK + mlsacNoisePitch;

            float maxV = (float)(minYawStep.getValue() * 0.52 + mlsacRng.nextDouble() * (maxYawStep.getValue() - minYawStep.getValue()) * 0.55);
            mlsacVelocityYaw   = MathHelper.clamp(mlsacVelocityYaw,   -maxV,        maxV);
            mlsacVelocityPitch = MathHelper.clamp(mlsacVelocityPitch, -maxV * 0.48f, maxV * 0.48f);
            ...
            // Human GCD error: người thật không on-grid hoàn toàn (~6-14% gcdFix)
            // gcdError=0 mọi tick là flag rõ ràng nhất của bot
            float humanJitterYaw   = (float)((mlsacRng.nextDouble() - 0.5) * gcdFix * 0.22);
            float humanJitterPitch = (float)((mlsacRng.nextDouble() - 0.5) * gcdFix * 0.14);

            rotationYaw   = rotationYaw   + gcdYaw   + humanJitterYaw;
            rotationPitch = MathHelper.clamp(rotationPitch + gcdPitch + humanJitterPitch, -90f, 90f);
        } else {
