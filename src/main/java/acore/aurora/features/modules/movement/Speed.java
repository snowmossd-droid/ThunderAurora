...
import acore.aurora.injection.accesors.IInteractionManager;
import acore.aurora.features.modules.Module;
import acore.aurora.setting.Setting;
import acore.aurora.utility.interfaces.IEntity;
...
import static acore.aurora.utility.player.MovementUtility.isMoving;
...
public class Speed extends Module {
    ...
    private acore.aurora.utility.Timer elytraDelay = new acore.aurora.utility.Timer();
    private acore.aurora.utility.Timer startDelay = new acore.aurora.utility.Timer();
    ...
    public void onSync(EventSync e) {
        ...
        if (mode.getValue() == Mode.MatrixJB) {
            ...
            if (MovementUtility.isMoving() && closeToGround && mc.player.fallDistance <= 0) {
                ...
                AcoreAurora.TICK_TIMER = matrixJBSpeed.getValue();
    ...
    public void modifyVelocity(EventPlayerTravel e) {
        if (mode.getValue() == Mode.GrimEntity && !e.isPre() && AcoreAurora.core.getSetBackTime() > 1000) {
        ...
        if ((mode.is(Mode.GrimEntity2) || mode.is(Mode.GrimCombo)) && !e.isPre() && AcoreAurora.core.getSetBackTime() > 1000 && MovementUtility.isMoving()) {
            int collisions = 0;
    ...
    public void onMove(EventMove event) {
        ...
        if (MovementUtility.isMoving()) {
            AcoreAurora.TICK_TIMER = useTimer.getValue() ? 1.088f : 1f;
