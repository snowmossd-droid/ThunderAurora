...
import acore.aurora.gui.font.Texture;
import acore.aurora.utility.math.MathUtility;
import acore.aurora.utility.render.shaders.*;
...
public class Render2DEngine {
    ...
    public static void drawHudBase(MatrixStack matrices, float x, float y, float width, float height, float radius) {
        if (HudEditor.hudStyle.is(HudEditor.HudStyle.Blurry)) {
            drawRoundedBlur(matrices, x, y, width, height, radius, new java.awt.Color(0,14,37,255));
        } else {
            BufferBuilder bb = preShaderDraw(matrices, x - 10, y - 10, width + 20, height + 20);
    ...
    public static void drawHudBase2(MatrixStack matrices, float x, float y, float width, float height, float radius, float blurStrenth, float blurOpacity, float animationFactor) {
        if (HudEditor.hudStyle.is(HudEditor.HudStyle.Blurry)) {
            ...
            Color c = Render2DEngine.interpolateColorC(Color.BLACK, new java.awt.Color(0,14,37,255), animationFactor);
    ...
    public static void drawTexturedRect(MatrixStack stack, net.minecraft.util.Identifier texture, float x, float y, float w, float h, int color) {
        ...
        net.minecraft.client.render.BufferBuilder buf = net.minecraft.client.render.Tessellator.getInstance()
            .begin(net.minecraft.client.render.VertexFormat.DrawMode.QUADS, net.minecraft.client.render.VertexFormats.POSITION_TEXTURE_COLOR);
        float r = ((color >> 16) & 0xFF) / 255f, g = ((color >> 8) & 0xFF) / 255f,
              b = (color & 0xFF) / 255f, a = ((color >> 24) & 0xFF) / 255f;
        buf.vertex(mat, x,     y,     0).texture(0, 0).color(r, g, b, a);
        buf.vertex(mat, x,     y + h, 0).texture(0, 1).color(r, g, b, a);
        buf.vertex(mat, x + w, y + h, 0).texture(1, 1).color(r, g, b, a);
        buf.vertex(mat, x + w, y,     0).texture(1, 0).color(r, g, b, a);
        endBuilding(buf);
        com.mojang.blaze3d.systems.RenderSystem.disableBlend();
    }

    public static void drawRoundBorder(MatrixStack stack, float x, float y, float w, float h, float radius, float thickness, Color color) {
        drawRound(stack, x, y, w, thickness, 0f, color);
        drawRound(stack, x, y + h - thickness, w, thickness, 0f, color);
        drawRound(stack, x, y, thickness, h, 0f, color);
        drawRound(stack, x + w - thickness, y, thickness, h, 0f, color);
    }

    public static void drawRound(MatrixStack stack, float x, float y, float w, float h, org.joml.Vector4f radius, Color color) {
        drawRound(stack, x, y, w, h, (radius.x + radius.y + radius.z + radius.w) / 4f, color);
    }

    public static void addWindow(MatrixStack stack, float x, float y, float x2, float y2, float scale) {
        addWindow(stack, new Rectangle(x, y, x2, y2));
    }
}
