...
public class Render3DEngine {
    ...
    public static void drawTargetEsp(MatrixStack stack, @NotNull Entity target) {
        ...
        for (int j = 0; j < vecs.size() - 1; ++j) {
            float alpha = 1f - (((float) j + ((System.currentTimeMillis() - AcoreAurora.initTime) / 5f)) % 360) / 60f;
    ...
    public static void renderGhostV1(net.minecraft.entity.LivingEntity target, float anim, float red, float speed) {
        if (target != null && mc.player != null) {
            ...
            double iAge = Render2DEngine.interpolate(mc.player.age - 1, mc.player.age, getTickDelta());
