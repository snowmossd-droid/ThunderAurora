...
import static acore.aurora.core.manager.client.ConfigManager.IMAGES_FOLDER;
...
public final class ThunderUtility {
    ...
    public static @NotNull String getAuthors() {
        List<String> names = AcoreAurora.MOD_META.getAuthors()
                .stream()
                .map(Person::getName)
                .toList();
    ...
    public static void syncVersion() {
        try {
            if (!new BufferedReader(new InputStreamReader(new URL("https://raw.githubusercontent.com/Pan4ur/THRecodeUtil/main/syncVersion121.txt").openStream())).readLine().equals(AcoreAurora.VERSION))
                AcoreAurora.isOutdated = true;
    ...
    public static void parseStarGazer() {
        ...
        try {
            for (int page = 1; page <= 3; page++) {
                URL url = new URL("https://api.github.com/repos/Pan4ur/AcoreAurora-Recode/stargazers?per_page=100&page=" + page);
    ...
    public static void syncContributors() {
        try {
            ...
            int i = 0;
            while ((inputLine = in.readLine()) != null) {
                AcoreAurora.contributors[i] = inputLine.trim();
                i++;
            }
            in.close();
        } catch (Exception e) {
    ...
    public static void parseCommits() {
        try {
            URL url = new URL("https://api.github.com/repos/Pan4ur/AcoreAurora-Recode/commits?per_page=50");
            BufferedReader in = new BufferedReader(new InputStreamReader(url.openStream(), StandardCharsets.UTF_8));

            changeLog.add("Changelog [Recode; Date: " + AcoreAurora.BUILD_DATE + "; GitHash:" + AcoreAurora.GITHUB_HASH + "]");
            changeLog.add("\n");
