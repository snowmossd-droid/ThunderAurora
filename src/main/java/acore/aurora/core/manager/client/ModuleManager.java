...
import acore.aurora.features.hud.impl.*;
import acore.aurora.features.modules.Module;
import acore.aurora.features.modules.client.*;
import acore.aurora.features.modules.combat.*;
import acore.aurora.features.modules.combat.MaceSwap;
import acore.aurora.features.modules.combat.ElytraSwapArmor;
import acore.aurora.features.modules.combat.ElytraAttack;
import acore.aurora.features.modules.misc.*;
import acore.aurora.features.modules.movement.Timer;
import acore.aurora.features.modules.movement.*;
...
public class ModuleManager implements IManager {
    ...
    public static ElytraAttack elytraAttack = new ElytraAttack();
    ...
    public static AcoreAuroraGui thunderHackGui = new AcoreAuroraGui();
    ...
    public void onLoad(String category) {
        try {
            AcoreAurora.EVENT_BUS.unsubscribe(unHook);
        ...
        modules.forEach(m -> {
            if (m.isEnabled() && (m.getCategory().getName().equalsIgnoreCase(category) || category.equals("none")))
                AcoreAurora.EVENT_BUS.subscribe(m);
        });
    ...
    public void onRender2D(DrawContext context) {
        ...
        AcoreAurora.core.onRender2D(context);
    }
    ...
    public void onUnload(String category) {
        modules.forEach(module -> {
            if (module.isEnabled() && (module.getCategory().getName().equalsIgnoreCase(category) || category.equals("none"))) {
                AcoreAurora.EVENT_BUS.unsubscribe(module);
    ...
    public void registerModule(Module module) {
        ...
        if (module.isEnabled())
            AcoreAurora.EVENT_BUS.subscribe(module);
    }
