package acore.aurora.core.manager.player;
...
import acore.aurora.events.impl.EventPostTick;
import acore.aurora.events.impl.PacketEvent;
import acore.aurora.events.impl.TotemPopEvent;
import acore.aurora.features.modules.Module;
import acore.aurora.features.modules.combat.AntiBot;
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof EntityStatusS2CPacket pac) {
            if (pac.getStatus() == EntityStatuses.USE_TOTEM_OF_UNDYING) {
                ...
                AcoreAurora.EVENT_BUS.post(new TotemPopEvent((PlayerEntity) ent, popList.get(ent.getName().getString())));
            }
        }
    }
